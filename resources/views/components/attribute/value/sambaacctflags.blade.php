...
<div class="input-group has-validation mb-3">
	<span @class(['btn-group btn-group-sm','is-invalid'=>($e=$errors->get($o->name_lc.'.'.$dotkey))]) role="group">
		@foreach($o::values as $k => $v)
			<input type="checkbox" class="btn-check" id="samba_af_{{ $k }}" name="{{ $o->name_lc }}[{{ Entry::TAG_INTERNAL }}][0][{{ $k }}]" value="1" @checked(($updated && $o->isset($k)) || old('sambaacctflags.'.Entry::TAG_INTERNAL.'.0.'.$k,$o->isset($k) && (! count(old())))) readonly>
			<label @class(['btn','btn-outline-dark'=>(! ($x=$o->isset($k))),'btn-dark'=>$x]) for="samba_af_{{ $k }}" data-bs-toggle="tooltip" data-bs-placement="bottom" title="{{ __($v) }}">{{ $k }}</label>
		@endforeach
	</span>
	...
	<script type="text/javascript">
		function sambaaccctflags() {
			...
			$(document).ready(function() {
				$('attribute#sambaacctflags input').on('click',function() {
					toggleSelect($(this).next('label'))
				});
