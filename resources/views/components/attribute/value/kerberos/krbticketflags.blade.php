...
<div class="input-group has-validation mb-1">
	<span @class(['btn-group btn-group-sm','is-invalid'=>($e=$errors->get($o->name_lc.'.'.$dotkey))]) role="group">
		@for($bit=31;$bit>=16;$bit--)
			...
			<label @class(['btn','btn-outline-dark'=>(! ($x=$o->isset($bit))),'btn-dark'=>$x,'readonly-checkbox'=>(! $edit) || ($template && $o->dn)]) for="kerb_tf_{{ $bit }}" data-bs-toggle="tooltip" data-bs-placement="bottom" title="{{ $helper->get($bit,__('Bit').' '.$bit) }}">{{ $x ? 1 : 0 }}</label>
		@endfor
	</span>
</div>
